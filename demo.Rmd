...
The input count matrix was provided as an excel file. Given that, this workflow imports the input matrix using the package [`readxl`](https://readxl.tidyverse.org/). Afterwards, the input data is cleaned by removing undetected genes and rounding each read count to an integer. As described in the [DESeq2 manual](https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#count-matrix-input), the finale shape of the input count matrix is to have columns corresponding to samples and rows corresponding to genes.

In order to build a DESeq2 object named `dds` from the input count matrix, a `colData` is also required. It consists of rows corresponding to samples and columns to conditions (e.g. genotype, treatment, replicate, etc) per sample. Importantly, row names of the `colData` and column names of the input count matrix have to match exactly when building a dds object.

Check out the references below for more info:

- [readxl doc](https://readxl.tidyverse.org/)
- [Bioconductor DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)
- [DESeq2 doc](https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html)
...
For convenience, QC is skipped in this workflow. Check out the [DESeq2 doc](https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html) or details about QC. For real-world analysis, QC using the function `vst()` is highly recommended in DESeq2-mediated RNA-seq.
...
## Calculating proportion of gene biotypes


```{r calculate_proportion, results='asis'}
...
## Plotting proportion of gene biotypes per condition

```{r plot_proportion, results='asis'}
